...
class LIDC(IMDB):
    ...
    def image_path_from_index(self, index):
        ...
        """
        image_file = os.path.join(self.data_path, 'JPEGImages', index + '.mat')
        assert os.path.exists(image_file), 'Path does not exist: {}'.format(image_file)
        return image_file
    def mask_path_from_index(self,index):
        mask_file = os.path.join(self.data_path,'MaskImages',index + '.mat')
        assert  os.path.exists(mask_file), 'Path does not exist: {}'.format(mask_file)
        return mask_file
    ...
    def load_pascal_annotation(self, index):
        ...
        roi_rec['mask'] = self.mask_path_from_index(index)
    ...
    def do_python_eval(self):
        ...
        :return: info_str
        """
        def check_in(pred_box,gt_box):
            cent_x = (pred_box[2] - pred_box[0] + 1) / 2
            cent_y = (pred_box[3] - pred_box[1] + 1) / 2
            if((gt_box[0]<cent_x<gt_box[2]) and (gt_box[1]<cent_y<gt_box[3])):
                return True
            return False
        with open(os.path.join(self.cache_path,'gt_roidb.pkl')) as f:
            roidb = cPickle.load(f)
        with open(os.path.join(self.result_path,'detection.okl')) as f:
            det = cPickle.load(f)
        num_rois = 0
        num_pred_rois = 0
        succ_pred = 0
        for idx,roi in enumerate(roidb):
            ...
            num_pred_rois += len(det_boxes)
            for gt_box in gt_boxes:
                for det_box in det_boxes:
                    if check_in(det_box,gt_box):
                        succ_pred += 1
        print(succ_pred/num_rois)
        print(succ_pred/num_pred_rois)
